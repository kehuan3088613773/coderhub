...
class MomentService {
  ...
  async hasLabel(momentId, labelId){
    const statment = `SELECT * FROM moment_label WHERE moment_id = ? AND label_id = ?;`
    const [result] = await connection.execute(statment, [momentId, labelId])
    return result[0] ? true : false
  }

  //添加动态标签关系
  async addLabel(momentId, labelId){
    const statment = `INSERT INTO moment_label (moment_id, label_id) VALUES (?, ?);`
    const [result] = await connection.execute(statment, [momentId, labelId])
    return result
  }
}
